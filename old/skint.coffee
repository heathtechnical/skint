class sAccount
    ...
    refreshDOM: (done) ->
        ...
        add_payment_row = (table, type, payment, skip_day) ->
            ...
            delete_button = 
                ...
                "class='btn btn-xs btn-danger accountPaymentsScheduledRemoveButton'><span class='glyphicon glyphicon-trash'></span></button>"
            ...
            tr = "<tr class='"+tr_class+"'>" + day +
                ...
                "</td><td>" + delete_button + "</td></tr>"

            $(table).append(tr)

        if @scheduled_payments
            $("#accountPaymentsScheduledPlaceholder").remove()
            add_payment_row("#accountPaymentsScheduled", "scheduled", item) for row, item of @scheduled_payments

        if @adhoc_payments
            $("#accountPaymentsAdHocPlaceholder").remove()
            add_payment_row("#accountPaymentsAdHoc", "adhoc", item, true) for row, item of @adhoc_payments
...
$ ->
    ...
    $("body").on "click", ".accountPaymentsScheduledRemoveButton", () ->
        ...
        $.post "/account/" + $("body").data("id") + "/update", delete_payment: payment, () ->
            sAccount.fetchAccount "54a714ca0d613b1153411300", (account) ->
                account.refreshDOM()
